...
<nav class="bg-gray-800 p-2 mt-0 fixed w-full z-10 top-0">
    <div class="container mx-auto flex flex-wrap items-center">
        ...
        <div
            class="flex w-full pt-2 md:pt-0 content-center justify-between md:w-1/2 md:justify-end">
            <ul class="list-reset flex justify-between flex-1 md:flex-none items-center">
                <?php
                foreach ($menu as $item) { ?>
                    <li class="mr-3">
                        <a
                            class="inline-block py-2 px-4 <?= $item['fileName']
                            === $fileName ? 'text-white no-underline'
                                : 'text-gray-600 no-underline hover:text-gray-200 hover:text-underline' ?>"
                            href="<?= $item['fileName'] ?>"
                        ><?= $item['name'] ?></a>
                    </li>
                <?php
